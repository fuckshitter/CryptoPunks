    ...
    const items = mutatedElements.map((value, i) => {
      ...
      if (cprice != cvalue) {
        ...
        while (s.length < 4) s = "0" + s;
          ...
          return ( <div class="card col-md-3"><Link to={newLinkUrl} className="nav-link"><img class="img-thumbnail" src={newImageUrl} id={value.index} onMouseEnter={this.callViewPunkDetail} onMouseOver={this.callViewPunkDetailOff} /></Link>  <div class="card-body"> {" "} <h5 class="card-title">NO {value.index}</h5> <p class="card-text"> PRICE {value.price} BNB</p> <Link to={newLinkUrl} className="nav-link"> BUY </Link> </div> </div> );
      }
    });
